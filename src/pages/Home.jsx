...
const Hero = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get('https://api.bhaskarai.com/api/feedback/testimonials/')
    ...
    >
      ...
      <div className="flex flex-col md:flex-row justify-between items-center w-full min-h-[600px]">
        ...
        >
          ...
          <h2 className="text-base sm:text-lg md:text-2xl mt-2 font-semibold text-gray-700 border-b border-orange-300 inline-block pb-1">
            {heroData?.title &&
              heroData.title.split(/,| and /).map((title, index, arr) => {
                const colors = [
                  "text-blue-600",
                  "text-purple-500",
                  "text-orange-400"
                ...
                return (
                  <React.Fragment key={index}>
                    <span className={colors[index] || "text-gray-500"}>
                      {title.trim()}
                    </span>
                    {index < arr.length - 1 && (
        ...
        <div className="w-full mt-10 overflow-hidden">
          <Marquee speed={40} pauseOnHover={true} gradient={false}>
            {testimonials.map((testimonial) => (
              <div
                key={testimonial.id}
                className="bg-white/10 backdrop-blur-sm border border-orange-300 rounded-lg p-4 text-center shadow-sm w-[300px] mx-4 text-sm text-white flex-shrink-0"
              >
                ...
                <div className="mt-2 flex flex-wrap items-center justify-center gap-1 text-xs">
                  <span className="text-orange-300 font-semibold">{testimonial.name}</span>
